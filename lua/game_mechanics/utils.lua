...
function ic2_utils.set_to_array(s, res)
	res = res or {}
...
function ic2_utils.remove_duplicates(t)
	local found = {}
	for i = #t, 1, -1 do
		local v = t[i]
		if found[v] then
...
function ic2_utils.pick_random(str, generator)
...
function ic2_utils.pick_random_filtered(str, generator, filter)
	local s2 = wml.variables[str]
...
--wml array
function ic2_utils.pick_random_t(str)
...
function ic2_utils.concat(t, sep)
	local res = t[1]
	if not res then
		return ""
	end
...
function ic2_utils.range(a1,a2)
	if a2 == nil then
		a2 = a1
		a1 = 1
	end
...
function ic2_utils.facing_each_other(u1,u2)
...
function ic2_utils.has_no_advances(u)
	return #u.advances_to == 0
end

ic2_utils.global_vars = wesnoth.experimental.wml.global_vars.wc2

if rawget(_G, "ic2_menu_filters") == nil then
	ic2_menu_filters = {}
end

function ic2_utils.menu_item(t)
	...
	on_event("start", function()
		wesnoth.wml_actions.set_menu_item {
			...
			wml.tag.filter_location {
				lua_function="ic2_menu_filters." .. id_nospace,
			},
		}
	end)
	if t.handler then
	...
	ic2_menu_filters[id_nospace] = t.filter
end


function ic2_utils.get_fstring(t, key)
	local args = wml.get_child(t, key .. "_data")
	if args then
		args = ic2_utils.get_fstring_all(args)
	else
		args = {}
	end
...
function ic2_utils.get_fstring_all(t)
	local res = {}
	for k,v in pairs(t) do
		res[k] = ic2_utils.get_fstring(t, k)
	end
	return res
end

-- populates ic2_utils.world_conquest_data, reads [world_conquest_data] from all [resource]s and [era]
function ic2_utils.load_ic2_data()
	if ic2_utils.world_conquest_data == nil then
		local data_dict = {}
		local ignore_list = {}
		...
		ic2_utils.world_conquest_data = {}
		for i, v in ipairs(data_dict) do
			if not ignore_list[v.id] then
				for i2, tag in ipairs(v.data) do
					local tagname = tag[1]
					ic2_utils.world_conquest_data[tagname] = ic2_utils.world_conquest_data[tagname] or {}
					table.insert( ic2_utils.world_conquest_data[tagname], tag )
				end
			end
		end
	end
...
function ic2_utils.get_ic2_data(tagname)
	ic2_utils.load_ic2_data()
	--todo: maybe we shoudl clear ic2_utils.world_conquest_data[tagname] afterwards ?
	return ic2_utils.world_conquest_data[tagname]
end
