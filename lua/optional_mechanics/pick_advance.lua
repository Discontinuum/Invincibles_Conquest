...
-- Works independed from the reset of the wc2 code (but needs ic2_utils.lua)
local on_event = wesnoth.require("on_event")
local ic2_utils = wesnoth.require("./../game_mechanics/utils.lua")
...
function pick_advance.has_options(u)
	return u and #u.advances_to > 1 and wml.variables.ic2_config_enable_pya
end
...
on_event("pre_advance", function(ec)
	...
	local picked = u.variables.ic2_pya_pick
	u.variables.ic2_pya_pick = nil
	if picked ~= nil then
		u.advances_to = { picked }
	end
end)

function wesnoth.wml_actions.ic2_pya_set_pick(cfg)
	local u = wesnoth.units.get(cfg.x, cfg.y)
	u.variables.ic2_pya_pick = cfg.pick
end

function wesnoth.wml_actions.ic2_pya_pick(cfg)
	local u = wesnoth.units.get(cfg.x, cfg.y)
	if not pick_advance.has_options(u) then
		return
	end
	...
	local message_wml = {
		...
		wml.tag.option {
			label = _"Random",
			image = ic2_color.tc_image("units/unknown-unit.png"),
			wml.tag.command {
				wml.tag.ic2_pya_set_pick {
					x=cfg.x,
					y=cfg.y,
				}
			}
	...
	for i,v in ipairs(options) do
		...
		table.insert(message_wml, wml.tag.option {
			label = ut.name,
			image = ic2_color.tc_image(ut.image),
			wml.tag.command {
				wml.tag.ic2_pya_set_pick {
					x=cfg.x,
					y=cfg.y,
...
ic2_utils.menu_item {
	...
	filter = function (x, y)
		if wml.variables.ic2_config_enable_pya == false then
			return false
		end
	...
	handler = function(cx)
		wesnoth.wml_actions.ic2_pya_pick {
			x=cx.x1,
			y=cx.y1,
		}
