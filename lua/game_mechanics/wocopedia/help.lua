...
local dialog = wml.load "~add-ons/Invincibles_Conquest_II/gui/help_dialog.cfg"
...
function wesnoth.wml_actions.ic2_show_wocopedia(cfg)
	...
	local preshow = function(dialog)
		...
		if show_help_training then
			...
			for i = 1, #ic2_training.get_list() do
				local current_level = ic2_training.get_level(current_side, i)
				local trainer = ic2_training.get_trainer(i)
				...
				local function set_description(train_num, j)
					local desc = ic2_training.generate_message(i, train_num)
		...
		if show_help_factions then
			...
			for i, faction_info in ipairs(ic2_era.factions_wml) do
				...
				page.label_deserters.marked_up_text = wesnoth.format_conjunct_list("", ic2_era.expand_hero_names(faction_info.deserters))
				page.label_commanders.marked_up_text = wesnoth.format_conjunct_list("", ic2_era.expand_hero_names(faction_info.commanders))
				page.label_heroes.marked_up_text = wesnoth.format_conjunct_list("", ic2_era.expand_hero_names(faction_info.heroes, true))
		...
		if show_help_artifacts then
			...
			for i, artifact in ipairs(ic2_artifacts.get_artifact_list()) do
		...
		if show_help_settings then
			...
			page.checkbox_use_pya.selected = not not wml.variables["ic2_config_enable_pya"]
			page.checkbox_use_pya.enabled = false
			page.checkbox_use_markers.selected = not not wml.variables["ic2_config_enable_unitmarker"]
			page.checkbox_use_markers.enabled = false
			page.checkbox_use_pickup.selected = not not wml.variables["ic2_config_experimental_pickup"]
			page.checkbox_use_pickup.enabled = false
			page.checkbox_show_pickup_confirmation.selected = not ic2_utils.global_vars.skip_pickup_dialog
			page.checkbox_show_pickup_confirmation.enabled = true

			page.label_version.marked_up_text = wml.variables["ic2_host_version"] or "unknown"
			page.label_difficulty.marked_up_text = wml.variables["ic2_difficulty.name"] or "unknown"

			function page.checkbox_show_pickup_confirmation.on_modified()
				ic2_utils.global_vars.skip_pickup_dialog = not page.checkbox_show_pickup_confirmation.selected
			end
		end
...
ic2_utils.menu_item {
	...
	handler = function(cx)
		wesnoth.wml_actions.ic2_show_wocopedia {
			x = cx.x1,
			y = cx.y1,
		}
	end
