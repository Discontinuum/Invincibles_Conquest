...
function artifacts.read_wml_data(cfg)
	for i, artifact in ipairs(ic2_convert.wml_to_lon(wml.literal(cfg), "wct_artifact_list").artifact or {}) do
		artifacts.add_artifact_data(artifact)
	end
end

function artifacts.init_data()
	local cfg = ic2_utils.get_ic2_data("artifact")
	for i, a in ipairs(ic2_convert.wml_to_lon(cfg, "wct_artifact_list").artifact or {}) do
...
function artifacts.drop_message(index)
	...
	wesnoth.wml_actions.message {
		...
		message = aftifact_data.info .. "\n" .. ic2_color.bonus_text(aftifact_data.description),
		image = aftifact_data.icon,
	}
end

function ic2_artifact_needs_compensation(side)
	return not ic2_scenario.is_human_side(side.side)
end


-- place an artifact with id @a index on the map at position @a x, y.
-- can be used from the bug console as `lua ic2_artifacts.place_item(30,20,1)`
function artifacts.place_item(x, y, index)
	wesnoth.wml_actions.item {
		...
		z_order = 20,
		wml.tag.variables { ic2_atrifact_id = index },
	}
end
...
function artifacts.give_item(unit, index, visualize)
	...
	local make_holder_loyal = wml.variables["ic2_config_items_make_loyal"] ~= false
	...
	if make_holder_loyal and (not unit.canrecruit) and (unit.upkeep ~= 0) and (unit.upkeep ~= "loyal") then
		unit:add_modification("object", { wml.tag.effect { apply_to = "ic2_overlay", add = "misc/loyal-icon.png" }})
	end

	local object = {
		ic2_atrifact_id = index,
		-- cannot filter on ic2_atrifact_id beeing empty so we also need ic2_is_artifact
		ic2_is_artifact = true,
	}
	...
	-- IDEA: i _could_ replace the follwing with a 'apply_to=ic2_artifact' effect that
...
on_event("ic2_drop_pickup", function(ec)
	local item = ic2_dropping.current_item
	local unit = wesnoth.units.get(ec.x1, ec.y1)
	if not item.variables.ic2_atrifact_id then
		return
	end
	...
	local is_human = ic2_scenario.is_human_side(side_num)
	if not wml.variables["ic2_config_experimental_pickup"] and not is_human  then
		return
	end
	

	local index = item.variables.ic2_atrifact_id
	...
	if is_human and not wml.variables["ic2_config_disable_pickup_confirm"] then
		if not ic2_pickup_confirmation_dialog.promt_synced(unit, artifacts.get_artifact(index).icon) then
			return
		end
	end
	

	ic2_dropping.item_taken = true
...
on_event("die", function(event_context)
	...
	if not wml.variables["ic2_config_experimental_pickup"] and ic2_scenario.is_human_side(unit.side) then
		return
	end
	for object in wml.child_range(wml.get_child(unit.__cfg, "modifications") or {}, "object") do
		if object.ic2_atrifact_id then
			artifacts.place_item(unit.x, unit.y, object.ic2_atrifact_id)
			artifacts.drop_message(object.ic2_atrifact_id)
			wesnoth.allow_undo(false)
		end
	end
	...
	unit:remove_modifications { ic2_is_artifact = true }
end)
...
function artifacts.is_item_at(x,y)
	for i,item in ipairs(wesnoth.interface.get_items(x,y)) do
		if item.variables.ic2_atrifact_id then
			return true
		end
	end
	return false
...
-- @a cfg.x, cfg.y
function wesnoth.wml_actions.ic2_show_item_info(cfg)
	local x = cfg.x
	local y = cfg.y
	for i,item in ipairs(wesnoth.interface.get_items(x,y)) do
		if item.variables.ic2_atrifact_id then
			local artifact_info = artifacts.get_artifact(item.variables.ic2_atrifact_id)
			wesnoth.wml_actions.message {
				scroll = false,
				image = artifact_info.icon,
				caption = artifact_info.name,
				message= artifact_info.info .. "\n" .. ic2_color.help_text(artifact_info.description),
			}
		end
	end
end
...
ic2_utils.menu_item {
	...
	handler = function(cx)
		wesnoth.wml_actions.ic2_show_item_info {
			x = cx.x1,
			y = cx.y1,
		}
	end
...
function wesnoth.wml_actions.ic2_place_item(cfg)
