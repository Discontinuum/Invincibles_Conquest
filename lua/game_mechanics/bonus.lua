...
function wesnoth.wml_actions.ic2_place_bonus(cfg)
	local x = cfg.x or helper.wml_error("[ic2_place_bonus] missing required 'x' attribute")
	local y = cfg.y or helper.wml_error("[ic2_place_bonus] missing required 'y' attribute")
	local scenery = cfg.scenery or helper.wml_error("[ic2_place_bonus] missing required 'scenery' attribute")
	local c_scenery = bonus.sceneries[scenery]
	if not c_scenery then
		helper.wml_error("[ic2_place_bonus] invalid 'scenery' attribute: ".. tostring(scenery))
		end
...
function bonus.place_item(x, y, image)
	...
	wesnoth.wml_actions.item {
		...
		image = image,
		z_order = 10,
		wml.tag.variables { ic2_is_bonus = true },
	}
end

function bonus.remove_current_item(ec)
	ic2_dropping.remove_current_item()
...
function bonus.can_pickup_bonus(side_num, x, y)
	return ic2_scenario.is_human_side(side_num)
end
...
on_event("ic2_drop_pickup", function(ec)
	...
	if not item.variables.ic2_is_bonus then
		return
	end
	...
	local bonus_type = item.ic2_type
	if bonus_type == nil then
		local training_chance = wml.variables.ic2_config_training_chance or 1
		local hero_chance = wml.variables.ic2_config_hero_chance or 1
		local item_chance = wml.variables.ic2_config_item_chance or 1
	...
	local bonus_subtype = item.ic2_subtype
	...
	assert(ic2_dropping.item_taken, "item still there")
end)

function bonus.get_random_item()
	return tonumber(ic2_utils.pick_random("wc2.random_items", ic2_artifacts.fresh_artifacts_list))
end

function bonus.get_random_hero(x, y)
	return ic2_utils.pick_random_filtered("wc2.random_heroes", ic2_era.generate_bonus_heroes, function(unittypeid)
		for _, sf in ipairs(ic2_era.spawn_filters) do
...
function bonus.found_hero(ec, herotype)
	...
	local newunit = ic2_heroes.place(herotype, finder.side, ec.x1, ec.y1)

	-- hero found and unit in bonus point face each other
	ic2_utils.facing_each_other(finder, newunit)
	ic2_heroes.founddialouge(finder, newunit)
...
function bonus.found_training(side_num, suggested_subtype, ec)
	...
	if suggested_subtype then
		amount = 1
		traintype = ic2_training.trainings_left(side_num, suggested_subtype) >= amount and suggested_subtype or nil
	else
		traintype, amount = ic2_training.pick_bonus(side_num)
	end
	...
	bonus.remove_current_item(ec)
	ic2_training.give_bonus(side_num, ec, amount, traintype)
	return true
end
