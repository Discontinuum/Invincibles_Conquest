...
local function show_dialog(unit, item_image)
	if ic2_utils.global_vars.skip_pickup_dialog then
		return 1
	end

	local dialog_wml = wml.load "~add-ons/Invincibles_Conquest_II/gui/pickup_confirmation.cfg"
	local function preshow(dialog)
		dialog.unit_icon.label = ic2_color.tc_image(wesnoth.unit_types[unit.type].image)
		dialog.arrow.label = ic2_color.tc_text(" → ")
