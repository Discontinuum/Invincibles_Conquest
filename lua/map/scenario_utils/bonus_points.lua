...
function get_f_wct_bonus_location_filter(map)
	local scenario_num = wml.variables.ic2_scenario or 1
	return f.all(
...
function world_conquest_tek_bonus_points(theme)
	...
	function place_item(loc)
		...
		table.insert(prestart_event, wml.tag.ic2_place_bonus {
			x= loc[1],
			y= loc[2],
			scenery = scenery,
		})
	...
	random_placement(possible_locs, wml.variables.ic2_player_count or 3, 9 + scenario_num, place_item)
	return res
end
