...
function wesnoth.effects.ic2_unitmarker(u, cfg)
	local number = math.min(cfg.number or 1, #img_is_special_overlays)
	
	u.variables["mods.ic2_unitmarker"] = number
...
function wesnoth.wml_actions.ic2_toggle_overlay(cfg)
	local units = wesnoth.units.find_on_map(cfg)
	for i, u in ipairs(units) do
		local overlay_nr = u.variables["mods.ic2_unitmarker"]
		if overlay_nr ~= nil then
			u:remove_modifications({ id = "ic2_unitmarker" })
		end
...
ic2_utils.menu_item {
	...
	filter = function(x, y)
		if wml.variables.ic2_config_enable_unitmarker == false then
			return false
		end
	...
	handler = function(cx)
		wesnoth.wml_actions.ic2_toggle_overlay {
			x = cx.x1,
			y = cx.y1,
		}
