...
local dialog_wml = wml.load "~add-ons/Invincibles_Conquest_II/gui/invest_dialog.cfg"

function ic2_show_invest_dialog_impl(args)
	...
	local function preshow(dialog)
		...
		if show_artifacts then			
			...
			for i,v in ipairs(available_artifacts) do
				local artifact_info = ic2_artifacts.get_artifact(tonumber(v))
				...
				local subnode, page = node:add_invest_item {
					...
					desc = ic2_color.tc_text(artifact_info.description),
		...
		if show_heroes then
			...
			if available_commanders then
				...
				local subnode, page = node:add_invest_item {
					icon = ic2_color.tc_image("units/unknown-unit.png"),
					name = _ "Commander" .. "\n" .. ic2_color.tc_text(_ "promote to leader"),
					result = { pick = "hero", type= "ic2_commander" }
				}
			...
			for j,v in ipairs(available_heroes) do
				...
				local subnode, page = node:add_invest_item {
					page_type = "hero",
					icon = ic2_color.tc_image(unit_type.image),
					name = unit_type.name,
			...
			if available_deserters then
				...
				local subnode, page = node:add_invest_item {
					icon = ic2_color.tc_image("units/unknown-unit.png"),
					name = _ "Deserter" .. "\n" .. ic2_color.tc_text("+15 gold"),
					result = { pick = "hero", type= "ic2_deserter" }
				}
				page.info_label.label = desc
			end
		end
		...
		if show_training then
			...
			for i,v in ipairs(available_training) do
				local current_grade = ic2_training.get_level(side_num, v)
				local training_info = ic2_training.get_trainer(v)
				local train_message = ic2_training.generate_message(v, current_grade + 1)
				local train_message_before = ic2_training.generate_message(v, current_grade)

				local title = stringx.vformat(_ "$name Training", { name = training_info.name })
				local desc = ic2_training.describe_training_level2(current_grade, #training_info.grade) .. ic2_color.tc_text(" → ") .. ic2_training.describe_training_level2(current_grade + 1, #training_info.grade)
				...
				page.info_label.label  = ic2_color.tc_text("<big>" .. _ "Before:" .. "</big>\n") .. train_message_before.message .. ic2_color.tc_text("\n<big>" .. _ "After:" .. "</big>\n") .. train_message.message
			end
		end
		...
		if show_other then
			...
			local colored_galleon = ic2_color.tc_image("units/transport/transport-galleon.png")
			local supplies_image = "misc/blank-hex.png~SCALE(90,80)~BLIT(" .. colored_galleon .. ",9,4)"
			local supplies_text = ic2_color.tc_text(_ "+70 gold and +1 village")
...
function ic2_show_invest_dialog(args)
	--do it in a loop to disable esc.
	while true do
		local d_res, res = ic2_show_invest_dialog_impl(args)
		if d_res ~= -2 then
			return res
		end
	end
