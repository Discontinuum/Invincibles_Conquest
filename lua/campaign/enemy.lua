...
function enemy.pick_suitable_enemy_item(unit)
	local enemy_items = stringx.split(wml.variables["ic2_enemy_army.artifacts"] or "")
	if #enemy_items == 0 then
		enemy_items = ic2_artifacts.fresh_artifacts_list("enemy")
	end
	-- list of indexes to enemy_items
	local possible_artifacts = {}
	for i, v in ipairs(enemy_items) do
		local filter = wml.get_child(ic2_artifacts.get_artifact(tonumber(v)), "filter")
	...
	wml.variables["ic2_enemy_army.artifacts"] = table.concat(enemy_items, ",")
	return artifact_id
end
...
on_event("recruit", function(ec)
	...
	local scenario_num = ic2_scenario.scenario_num()
	...
	ic2_artifacts.give_item(unit, item_id, false)
	if true then
		unit.experience = unit.experience + scenario_num  * (16 + wml.variables["ic2_difficulty.enemy_power"])
...
function enemy.do_commander(cfg, group_id, loc)
	...
	local scenario = ic2_scenario.scenario_num()
	--wesnoth.interface.add_chat_message("do_commander", wml.variables[("ic2_enemy_army.group[%d].allies_available"):format(group_id)])
	local ally_i = ic2_utils.pick_random(("ic2_enemy_army.group[%d].allies_available"):format(group_id)) - 1
	local leader_index = mathx.random(wml.variables[("ic2_enemy_army.group[%d].leader.length"):format(ally_i)]) - 1
	local new_recruits = wml.variables[("ic2_enemy_army.group[%d].leader[%d].recruit"):format(ally_i, leader_index)]
	wesnoth.wml_actions.allow_recruit {
		side = cfg.side,
		type = new_recruits
	}
	local commander_options = wml.variables[("ic2_enemy_army.group[%d].commander.level%d"):format(ally_i, cfg.commander)]
	wesnoth.wml_actions.unit {
		...
		generate_name = true,
		role = "commander",
		experience = scenario * ((wml.variables["ic2_difficulty.enemy_power"] or 6) - 7 + cfg.commander),
		wml.tag.modifications {
				ic2_heroes.commander_overlay_object(),
				wml.tag.trait(ic2_heroes.trait_heroic),
		},
	}
end
...
function enemy.do_supply(cfg, group_id, loc)
	...
	u:add_modification("trait", ic2_heroes.trait_expert)

	wesnoth.wml_actions.event {
		name = "side " .. cfg.side .. " turn 2",
		wml.tag.ic2_map_supply_village {
			x = u.x,
			y = u.y,
		}
	}
...
function enemy.do_recall(cfg, group_id, loc)
	...
	local group = wml.variables[("ic2_enemy_army.group[%d]"):format(group_id)]
	local to_recall = stringx.split(side_variables["wc2.to_recall"] or "")
...
function enemy.do_training(cfg, group_id, loc)
	...
	local dif = wml.variables["ic2_difficulty.enemy_trained"] or 0
	if tr ~= 0 and dif >= tr then
		--enemy can only get Melee, Ranger, Health or Movement
		wesnoth.wml_actions.ic2_give_random_training {
			side = cfg.side,
			among="2,3,4,6"
		}
	end
...
function enemy.do_gold(cfg, side)
	...
	local enemy_power = (wml.variables["ic2_difficulty.enemy_power"] or 6)
	local factor = (cfg.nplayers + 1 ) * enemy_power/6 - 2
	side.gold = side.gold + cfg.bonus_gold * factor
end

function enemy.init_data()
	if wml.variables.ic2_enemy_army == nil then
		-- give eras an option to overwrite the enemy data.
		wesnoth.fire_event("ic2_init_enemy")
	end
	if wml.variables.ic2_enemy_army == nil then
		-- give eras an option to overwrite the enemy data.
		local enemy_army = wesnoth.dofile("./enemy_data.lua")
		wml.variables.ic2_enemy_army = ic2_convert.lon_to_wml(enemy_army, "wct_enemy")
	end
end
...
--[[
	...
	[/ic2_enemy]
--]]
function wesnoth.wml_actions.ic2_enemy(cfg)
	enemy.init_data()
	local side_num = cfg.side
	local side = wesnoth.sides[side_num]
	local scenario = ic2_scenario.scenario_num()
	...
	dummy_unit:erase()
	local enemy_type_id = ic2_utils.pick_random("ic2_enemy_army.factions_available") - 1
	if enemy_type_id == nil then
		--should't happen, added for robustness.
		local n_groups = wml.variables["ic2_enemy_army.group.length"]
		if n_groups > 0 then
	...
	local unit = wesnoth.units.create {
		...
		generate_name = true,
		max_moves = 0,
		wml.tag.modifications { wml.tag.trait(ic2_heroes.trait_heroic) },
	}
	if unit.name == "" then
		-- give names to undead
		unit.name = ic2_random_names()
	end
	unit:to_map()
	wesnoth.wml_actions.set_recruit {
		side = side_num,
		recruit = wml.variables[("ic2_enemy_army.group[%d].recruit"):format(enemy_type_id)]
	}
	wesnoth.wml_actions.allow_recruit {
		side = side_num,
	...
	if cfg.have_item > 0 and cfg.have_item <= (wml.variables["ic2_difficulty.enemy_power"] or 6) then
