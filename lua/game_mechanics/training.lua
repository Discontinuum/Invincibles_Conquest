...
function training.read_wml_data(cfg)
	for i, t in ipairs(ic2_convert.wml_to_lon(cfg, "wct_trainer_list").trainer or {}) do
...
function training.init_data()
	local cfg = ic2_utils.get_ic2_data("trainer")
	for i, t in ipairs(ic2_convert.wml_to_lon(cfg, "wct_trainer_list").trainer or {}) do
		training.add_training_data(t)
	end
end
...
function training.list_available(side_num, among, amount)
	local av = among or ic2_utils.range(#training.get_list())
	local res = {}
	...
	for unused, chance in ipairs(c_grade.chance) do
		...
		if (chance.value or 0) < 100 then
			local str = stringx.vformat(_ "$chance| chance to $arrow $desc", {
				...
				desc = ic2_utils.get_fstring(chance, "info"),
				arrow = ic2_color.tc_text(" → ")
			})
			table.insert(messages, tostring(str))
		else
			table.insert(messages, tostring(ic2_utils.get_fstring(chance, "info")))
		end
	end
	return {
		caption = caption,
...
function training.give_bonus(side_num, cx, amount, traintype_index)
	...
	local teacher = ic2_heroes.place(amount > 1 and traintype.advanced_type or traintype.type, side_num, cx.x1,cx.y1)
	local u = wesnoth.units.get(cx.x1, cx.y1)
	ic2_utils.facing_each_other(u, teacher)
...
function training.bonus_calculate_amount(side_num)
	...
	if ic2_scenario.scenario_num() > 3 or mathx.random(100) <= advanced_chance then
		amount = 2
	end
	return amount
end
...
function training.pick_bonus(side_num)
	...
	if traintype_index == 1 then
		amount = math.min(training.trainings_left(side_num, traintype_index), math.max(amount, ic2_scenario.scenario_num() - 1))
	end
...
function training.apply(u)
	...
	for i, trainer in ipairs(training.get_list()) do
		...
		for unused, chance in ipairs(training.get_chances(i, level)) do
			...
			if mathx.random(100) <= vchance.value and matches_filter then
				...
				table.insert(descriptions, ic2_utils.get_fstring(chance, "info"))
	...
	trait.description = ic2_utils.concat(descriptions, "\n")
	if #trait > 0 then
		u:add_modification("trait", trait)
		--rebuild unit, to reduce savefile size.
		u:transform(u.type)
	end
	u.variables.ic2_trained = true
	u.hitpoints = u.max_hitpoints
end

function wesnoth.wml_actions.ic2_apply_training(cfg)
	for i,u in ipairs(wesnoth.units.find_on_map(cfg)) do
		training.apply(u)
	end
end

function wesnoth.wml_actions.ic2_give_random_training(cfg)
	...
	for i = 1, amount do
		...
		if traintype == nil then error("ic2_give_random_training: everything alerady maxed") end
